...
<?php if ($strategy->isRejected() && $strategy->moderation_note): ?>
  <div class="w-full max-w-[1366px] mx-auto px-4 md:px-0 mb-6">
    <div class="flex gap-3 p-4 rounded-md bg-error-base/10 border border-error-base">
...
<?php elseif ($strategy->isPending()): ?>
  <div class="w-full max-w-[1366px] mx-auto px-4 md:px-0 mb-6">
    <div class="flex gap-3 p-4 rounded-md bg-gray-2 border border-gray-3">
      <i class="ph ph-clock text-gray-5 text-2xl shrink-0" aria-hidden="true"></i>
      <p class="text-gray-6 font-nunito text-sm">Esta estratégia ainda está em análise. Editar e salvar reinicia a fila de revisão.</p>
    </div>
  </div>
<?php endif; ?>
...
<form action="/strategy-edit?id=<?= e($strategy->id) ?>" method="post" enctype="multipart/form-data" novalidate
  ...
  <div class="flex flex-col gap-6 w-full md:w-auto">
    ...
    <div>
      <label for="image-input" id="image-upload-box"
        ...
        <span class="upload-placeholder flex flex-col items-center justify-center pointer-events-none <?= $strategy->cover_image_url ? 'hidden' : '' ?>">
        ...
        <span id="image-preview-box" class="absolute inset-0 <?= $strategy->cover_image_url ? '' : 'hidden' ?>">
          <img id="image-preview" src="<?= e((string) $strategy->cover_image_url) ?>"
            alt="Pré-visualização da capa" class="w-full h-full object-cover rounded-[18px]">
        </span>
    ...
    <div>
      <label for="video-input" id="video-upload-box"
        class="relative overflow-hidden cursor-pointer w-full max-w-[381px] h-44 sm:h-[240px] md:w-[381px] md:h-[240px] flex flex-col items-center justify-center rounded-[18px] bg-gray-3 border-2 border-gray-3 hover:border-red-base focus-within:border-red-base transition-all ease-in-out duration-300">
        <span class="upload-placeholder flex flex-col items-center justify-center pointer-events-none <?= $strategy->video_url ? 'hidden' : '' ?>">
  ...
  <div class="flex flex-col justify-between w-full mt-6 md:mt-0">
    <div>
      <h1 class="font-rajdhani font-bold text-2xl md:text-xl text-gray-7">Editar estratégia</h1>
