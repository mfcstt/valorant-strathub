<a href="/strategy?id=<?= $estrategia->id ?>" class="estrategiaCard group relative w-full h-[280px] rounded-xl outline-none focus:outline-red-base transition-all ease-in-out duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-red-base/20">
  ...
  <div class="absolute w-full h-full overflow-hidden rounded-xl">
    <?php if ($estrategia->cover_image_url): ?>
      <img src="<?= $estrategia->cover_image_url ?>" alt="Capa da estratégia" loading="lazy" decoding="async" class="w-full h-full object-cover group-hover:scale-110 group-focus:scale-110 transition-transform duration-[400ms]">
    <?php elseif ($estrategia->video_url): ?>
       <video data-src="<?= $estrategia->video_url ?>" class="w-full h-full object-cover group-hover:scale-110 group-focus:scale-110 transition-transform duration-[400ms] video-cover lazy-video" preload="none" muted playsinline></video>
