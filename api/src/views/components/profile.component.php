  ...
  <section class="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8">
    ...
    <div class="p-6 rounded-[18px] bg-gray-2">
      <div class="flex flex-col items-center gap-4">
        ...
        $eloSlug = strtolower($user->elo ?? 'ferro');
        ?>
        ...
        <form action="/profile" method="post" enctype="multipart/form-data" class="w-full">
          ...
          <button type="submit"
            class="mt-4 w-full px-5 py-2 rounded-md text-white font-nunito bg-red-base outline-none hover:bg-red-light focus:bg-red-light focus:outline-red-base transition-all ease-in-out duration-300">Atualizar
            avatar</button>
        </form>
      </div>
    </div>
    ...
    <div class="p-6 rounded-[18px] bg-gray-2">
      <form action="/profile" method="post" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="flex items-end">
          <button type="submit"
            class="px-5 py-2 rounded-md text-white font-nunito bg-red-base outline-none hover:bg-red-light focus:bg-red-light focus:outline-red-base transition-all ease-in-out duration-300">Salvar
            alterações</button>
        </div>
      </form>
      ...
      <div class="mt-6 flex items-center gap-4">
        <a href="/myStrategy"
          class="flex items-center gap-2 px-5 py-2 rounded-md text-white font-nunito bg-red-base outline-none hover:bg-red-light focus:bg-red-light focus:outline-red-base transition-all ease-in-out duration-300">
      ...
      <div class="mt-10">
        ...
        <form action="/profile" method="post" class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="flex items-end md:col-span-2">
            <button type="submit"
              class="px-5 py-2 rounded-md text-white font-nunito bg-red-base outline-none hover:bg-red-light focus:bg-red-light focus:outline-red-base transition-all ease-in-out duration-300">Alterar
              senha</button>
          </div>
        </form>
      </div>
      ...
      <div class="mt-10">
        ...
        <p class="text-gray-6 font-nunito text-sm mb-4">Essa ação é permanente e removerá suas estratégias e avaliações
          associadas.</p>
        <form action="/profile" method="post" class="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6">
          ...
          <div class="flex items-end">
            <button type="submit"
              class="px-5 py-2 rounded-md text-white font-nunito bg-red-base outline-none hover:bg-red-light focus:bg-red-light focus:outline-red-base transition-all ease-in-out duração-300">Apagar
              conta</button>
          </div>
        </form>
      </div>
...
<script>
  (function () {
    const sel = document.querySelector('select[name="elo"]');
    const img = document.getElementById('eloPreview');
    if (sel && img) {
      const update = () => {
        const val = (sel.value || '').toLowerCase();
        if (val) {
          img.src = '/assets/images/elos/' + val + '.png';
          img.alt = 'Elo selecionado: ' + val.charAt(0).toUpperCase() + val.slice(1);
          img.style.display = '';
        } else {
