<div class="px-4 md:px-8 lg:px-16 xl:px-24 pt-16 pb-5"> 
  <form class="w-full flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8" method="get" novalidate>
    <h1 class="font-rammetto text-2xl text-[#E5E2E9]">Favoritas</h1>
    ...
    <div class="flex flex-col sm:flex-row gap-4 w-full md:w-auto relative">
      ...
      <?php 
        ...
        $orderIconClass = $orderIconMap[$order ?? 'recentes'] ?? 'ph ph-clock';
      ?>
      <div class="sm:w-56 relative flex items-center">
        ...
        <select name="ordenar" class="inpForm pl-10" onchange="this.form.submit()">
          ...
          <option value="recentes" <?= ($order ?? 'recentes') === 'recentes' ? 'selected' : '' ?>>Recentes</option>
        </select>
      </div>

      <?php $hasActiveFiltersFav = !empty($filter_category ?? '') || !empty($filter_agent ?? '') || !empty($filter_map ?? ''); ?>
      <button type="button" id="toggleFiltersFavorites" class="flex items-center justify-center gap-2 px-5 py-3 rounded-md text-white font-nunito bg-gray-1/80 border <?= $hasActiveFiltersFav ? 'border-red-base text-red-light' : 'border-gray-3 text-gray-5' ?> outline-none hover:text-red-light hover:border-red-base focus:text-red-light focus:outline-red-base transition-all ease-in-out duration-300">
        <i class="<?= $hasActiveFiltersFav ? 'ph-fill ph-funnel-simple' : 'ph ph-funnel-simple' ?> text-xl"></i>
        Filtrar
      </button>
      ...
      <div id="filtersPanelFavorites" class="hidden absolute top-full right-0 z-50 w-[92vw] max-w-md p-4 rounded-md bg-gray-3 border border-gray-4 shadow-xl">
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="/favorites" class="flex items-center gap-2 px-5 py-3 rounded-md text-gray-5 font-nunito bg-gray-1/80 border border-gray-3 outline-none hover:text-red-light hover:border-red-base focus:text-red-light focus:outline-red-base transition-all ease-in-out duration-300">
    ...
    <div id="filtersBackdropFavorites" class="hidden fixed inset-0 z-40"></div>
  </form>
  ...
  <?php if (($total_pages ?? 1) > 1): ?>
    ...
    <nav class="mt-6 flex items-center justify-center gap-3">
      <a href="/favorites?<?= $prevQuery ?>" class="px-4 py-2 rounded-md bg-gray-1/80 border border-gray-3 text-gray-5 outline-none hover:text-red-light hover:border-red-base focus:text-red-light focus:outline-red-base transition-all">Anterior</a>
      <span class="px-3 py-2 text-gray-6">Página <?= $page ?? 1 ?> de <?= $total_pages ?></span>
      <a href="/favorites?<?= $nextQuery ?>" class="px-4 py-2 rounded-md bg-gray-1/80 border border-gray-3 text-gray-5 outline-none hover:text-red-light hover:border-red-base focus:text-red-light focus:outline-red-base transition-all">Próxima</a>
    </nav>
  <?php endif; ?>
  ...
  <?php if (!$estrategias): ?>
    <div class="flex flex-col gap-5 items-center text-center font-nunito">
      ...
      <a href="/favorites" class="flex items-center gap-2 text-gray-5 outline-none hover:text-red-light focus:text-red-light transition-all ease-in-out duration-300">
  ...
  <script>
    (function(){
      const btn = document.getElementById('toggleFiltersFavorites');
      const panel = document.getElementById('filtersPanelFavorites');
      const backdrop = document.getElementById('filtersBackdropFavorites');
